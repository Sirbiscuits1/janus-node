...
export const checkLimits = ({ body, quote, rates }) => {
  ...
  if (Number.isFinite(inputTokens) && Number.isFinite(rates.contextTokens)) {
    ...
    if ((inputTokens + ceiling) > rates.contextTokens) {
      // Say WHICH part is too big. Now that tool schemas are counted, a caller
      // can be refused by a prompt that looks short to them — and "your prompt
      // is too long" when the messages are three lines reads as a bug in us.
      const toolTokens = quote?.breakdown?.toolTokens ?? 0
      const where = toolTokens > 0
        ? ` (${inputTokens - toolTokens} in messages, ${toolTokens} in tool definitions)`
        : ''

      return err(
        ERR_CONTEXT_TOO_LONG,
        `Prompt plus answer would need ${inputTokens + ceiling} tokens${where}; ` +
        `listing offers ${rates.contextTokens}.`,
        { contextTokens: rates.contextTokens, inputTokens, toolTokens }
      )
    }
  }
